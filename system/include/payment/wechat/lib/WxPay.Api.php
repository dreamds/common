...
class WxPayApi
{
	...
	public static function refund($inputObj, $timeOut = 30)
	{
		...
		// $response = self::postXmlCurl($xml, $url, true, $timeOut); // 旧的方法会有pem验证不通过问题等
		// 用新的方法处理
		$response = self::myPostXmlCurl($xml, $url, true, $timeOut);
		$result = WxPayResults::Init($response);
	 ...
	 * @return xml
	 */
	private function myPostXmlCurl($xml, $url, $useCert = false, $second = 30, $aHeader = array())
	{
		$ch = curl_init();
		//超时时间
		curl_setopt($ch, CURLOPT_TIMEOUT, $second);
		...
		if (true == $useCert) {
			// 设置证书绝对路径
			$wechat_base_dir = dirname(dirname(__FILE__));
			$sslcert_path    = $wechat_base_dir . '/' . str_replace('../', '', WxPayConfig::SSLCERT_PATH);
			$sslkey_path     = $wechat_base_dir . '/' . str_replace('../', '', WxPayConfig::SSLKEY_PATH);
		...
		if($data){
			...
			$error = curl_errno($ch);
			curl_close($ch);
			throw new WxPayException("call faild, errorCode:$error");
		}
	}
