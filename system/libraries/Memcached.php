...
class CI_Memcached {
    ...
    public function __construct($config = array())
    {
        ...
        // 如果是 windows 系统是没有 php_memcached.dll 扩展的
        if ('win' === strtolower(substr(PHP_OS, 0, 3))) {
            $this->__extension = 'memcache';
        }
     ...
     * @return  bool
     */
    public function cas($cas_token, $key, $data, $expire)
    {
        return $this->__memcached->cas($cas_token, $key, $data, $expire);
    }
     ...
     * @return  mixed
     */
    public function get($key, $callable = null, &$cas_token = null)
    {
        if ('memcache' == $this->__extension) {
            return $this->__memcached->get($key);
        } else if ('memcached' == $this->__extension) {
            return $this->__memcached->get($key, $callable, $cas_token);
        } else {
            return false;
        }
     ...
     * @return  bool
     */
    public function lock($key, $data, $expire)
    {
        return $this->add($key, $data, $expire);
    }
     ...
     * @return  bool
     */
    public function unlock($key, $time = 0)
    {
