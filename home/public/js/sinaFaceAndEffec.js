...
var face_icon_obj = null;  //初始化表情按钮对象
...
(function($){
	$.fn.SinaEmotion = function(target){
		var text_obj = target.attr('id');  //要显示表情的区域的id(这里用id标识唯一)
		var cat_current;
		var cat_page;
		$(this).click(function(event){
			face_icon_obj = $(this);
			event.stopPropagation();
			var eTop  = face_icon_obj.offset().top;
			var eLeft = face_icon_obj.offset().left-3;
		...
		function showEmotions(){
			...
			$('#emotions .container a').click(function(){
				face_icon_obj.parent().parent().parent().find('#'+text_obj).insertText($(this).attr('title'));
				$('#emotions').remove();
			});
